...
var MultifieldShape_1 = require("./MultifieldShape");
describe('converter to react-docgen syntax', function () {
    ...
    it('Should convert component with nested interface', function () {
        var fileName = path.join(__dirname, '../../src/__tests__/data/Multifield.tsx'); // it's running in ./temp
        var result = docgenConverter_1.convertToDocgen(parser_1.getDocumentation(fileName));
        chai_1.assert.deepEqual(result.props, MultifieldShape_1.default);
    });
});
