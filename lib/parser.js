...
function getDocumentation(fileName, options) {
    ...
    function getType(prop) {
        ...
        if (typeAtLocation) {
            var declaredType = checker.getTypeAtLocation(prop.type).intrinsicName;
            if (declaredType)
                return { type: declaredType };
            var multipleTypes = checker.getTypeAtLocation(prop.type).types;
            if (multipleTypes)
                return { type: 'enum', values: multipleTypes.map(function (n) { return "\"" + n.text.toString() + "\""; }) };
        }
